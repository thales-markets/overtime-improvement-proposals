...
This multiplier was selected to approximate the **average protocol margin per leg**, compensating liquidity providers for assuming the remaining market risk when purchasing a ticket early.
...
The cashout multiplier is calculated as:
...
The additional `+1` term reflects the protocol’s assumption of ticket risk during the cashout transaction itself.

This effectively creates a **minimum cashout fee close to 8%** even when only a single leg remains (e.g. with `v = 4%`, the effective fee for `n = 1` is `1 − 0.96^2 ≈ 7.84%`), helping mitigate potential frontrunning of live odds while the system matures.

### Effective fee examples

With `SafeBoxFee ≈ 1%` and `v = 4%`, the resulting effective fees are approximately:

| Remaining Legs (n) | Effective Fee `1 − (1 − v)^(n + 1)` |
|---:|---:|
| 1 | ~7.84% |
| 2 | ~11.53% |
| 3 | ~15.07% |
...
Original probabilities: `0.50 × 0.50`  
`P_orig = 0.25`

Remaining live probability: `0.70`  
`R = 0.70 / 0.25 = 2.8`

Fee: `v = 4%`, `n = 1`  
`KeepMultiplier = 0.96^(1+1) = 0.96^2 = 0.9216`

`CashoutQuote = 2.8 × 0.9216 = 2.58048`  
Cashout payout ≈ `100 × 2.58048 = 258.05 USDC`

---

### Example 2 — Odds worsen

Using the same original probabilities (`P_orig = 0.25`), remaining live probability: `0.30`

`R = 0.30 / 0.25 = 1.2`

Fee: `v = 4%`, `n = 1`  
`KeepMultiplier = 0.96^2 = 0.9216`

`CashoutQuote = 1.2 × 0.9216 = 1.10592`  
Cashout payout ≈ `110.59 USDC`

---

### Example 3 — Two legs remaining

Original: `0.6 × 0.5 × 0.5`  
`P_orig = 0.15`

Live remaining: `0.65 × 0.55`  
`P_live = 0.3575`

`R = 0.3575 / 0.15 ≈ 2.38333`

`n = 2`  
`KeepMultiplier = 0.96^(2+1) = 0.96^3 = 0.884736`

`CashoutQuote ≈ 2.38333 × 0.884736 ≈ 2.10862`  
Cashout payout ≈ `210.86 USDC`

This demonstrates compounding fee behavior for longer parlays.
...
Cashout vig calculations and fee parameters are **fully configurable by pDAO** and may be adjusted over time as the protocol gathers real-world usage data.

Governance may modify:

- the cashout SafeBox multiplier
- fee scaling across remaining legs
- operational/risk constraints around cashout execution

This flexibility ensures the system can evolve toward **optimal pricing and LP protection** while maintaining a competitive user experience.
